---
name: jira-card
description: One-command Jira card workflow for any project — `/jira-card "<what to do>"` creates a well-formed card, resolving board, sprint, due date, story points and priority from natural language plus a per-repo config, asking (via a structured prompt) only when genuinely unclear. The same command also routes to backfill (cards from git commits), implement (build a backlog card), browse, setup, and list. First run auto-discovers everything into `.claude/jira-config.json` via the Atlassian MCP, asking at most one question — no hardcoded instance IDs. Exactly one confirmation before any write; never moves a card to Done without explicit approval; never auto-commits code. All command words are lowercase. Trigger on `/jira-card`, `/jira`, "สร้างการ์ด", "ทำ card", "backfill jira", "ปิด sprint", "ดึงการ์ด", "ทำ backlog", "ดู backlog", "implement card", "ตั้งค่า jira", and proactively when a repo's commits outrun the Jira board or the user starts work matching an open To-Do card.
---
